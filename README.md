...
基于 SpringBoot 2.0 + Layui2 框架的物流管理系统，采用前后端分离。在此基础上，我修改了几个明显的bug和优化了一些代码。
最后非常感谢anselleeyy，他提供了一个完整的物流逻辑，才会使我们减少了大量的调查工作。
- 原项目地址：https://github.com/anselleeyy/logistics-back
...
- 1：添加Lombok，提高实体类的简洁性。
- 2：用户组权限修改时，只能添加权限，不能删除权限。
- 3: 录入职工薪资时，一个员工会多次加入数据库。
- 4：添加日期转换器
- 5：boot启动配置类优化。
- 6：中转管理优化（可有多个中转城市，填写多次中转回告）
- 7：添加回告 ，添加邮箱，短信操作
- 8: 统计分析加入 EChart，提高用户体验。 
- 9： 使用redis 缓存权限数据。
 
欢迎大家多多交流，提高这个项目的技术含量。欢迎提交issue给我，我会尽快处理
...
> 作者是应届毕业生，该项目的原型来自github，为了完成毕业设计，所以，我在作者原来的基础上，优化了一些bug和逻辑。但整体上没有改动太多。
> 另外，我觉得这个项目中的数据库设计有些怪异，可能是为了使用JPA，所以数据表较多。
> 最后，原系统中，逻辑处理不是太严谨，应尽量避免空指针。  
